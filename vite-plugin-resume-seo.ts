      ...
      if (exp.details?.tasks) {
        ...
        if (tasks && tasks.length > 0) {
          ...
          lines.push(`${indent}      </ul>`)
            ...
            if (exp.details?.features) {
        const features = exp.details.features[config.languages.default] ?? Object.values(exp.details.features)[0]
        if (features && features.length > 0) {
          lines.push(`${indent}      <ul style="margin: 0.5rem 0 0 1rem; padding: 0;">`)
          for (const feature of features) {
            lines.push(`${indent}        <li style="margin-bottom: 0.15rem; font-size: 0.9rem;">${escapeHtml(feature)}</li>`)
          }
          lines.push(`${indent}      </ul>`)
        }
      }
